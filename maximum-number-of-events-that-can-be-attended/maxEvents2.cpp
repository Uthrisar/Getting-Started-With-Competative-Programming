...
public:
    int maxValue(vector<vector<int>>& events, int k) {
        ...
        return rec(n-1,k,endPoints,events,dp);
    }
    int rec(int ind,int k,vector<int>& endPoints,vector<vector<int>>& events,vector<vector<int>>& dp){
        if(k == 0) return 0;
        if(ind < 0) return 0;
        if(dp[ind][k] != -1) return dp[ind][k];
        int ans = rec(ind-1,k,endPoints,events,dp);
        auto it = upper_bound(endPoints.begin(),endPoints.end(),events[ind][0]);
        if(it != endPoints.begin()){
            it--;
            int j = distance(endPoints.begin(),it);
            ans = max(ans,rec(j,k-1,endPoints,events,dp)+events[ind][2]);
        } else {
